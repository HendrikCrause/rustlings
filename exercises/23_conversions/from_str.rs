...
impl FromStr for Person {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        match (name, age, rest) {
            (_, None, _) | (None, _, _) | (_, _, Some(_)) => Err(ParsePersonError::BadLen),
            (Some(""), _, _) => Err(ParsePersonError::NoName),
            (_, Some(Err(p)), _) => Err(ParsePersonError::ParseInt(p)),
            (Some(n), Some(Ok(a)), None) => Ok(Person{name: n.to_string(), age: a}),
        }
